...
numeric accessors `get_int` / `get_uint` / `get_double`: glued trailing
...
the root handle from `Parser.iter(...)`, navigates with `find_field(key)` /
`get_array` / `next_element`, and reads a leaf without naming any `[o]`-parametric
type. The bare-root-at-EOF case (a number that is the entire document) cannot be
reached through the object-only `Parser.iter(...)`, so it is NOT exercised here;
it is covered by M4's validate() path. The non-EOF terminator path is covered by
the `}` and `,` terminator cases below.
"""
...
def test_get_double_double_dot_raises() raises:
    """A number with a glued second dot (12.3.4) makes get_double raise."""
    var data = _make_bytes(String('{"n":12.3.4}'))
    var parser = Parser()
    var root = parser.iter(data)
    var raised = False
    try:
        _ = root.find_field(String("n")).get_double()
    except:
        raised = True
    assert_true(raised, "get_double on 12.3.4 must raise (trailing junk)")


def test_get_double_one_dot_two_dot_three_raises() raises:
    """A number with a glued second dot (1.2.3) makes get_double raise."""
    ...
    try:
        ...
        raised = True
    assert_true(raised, "get_double on 1.2.3 must raise (trailing junk)")


def test_get_double_double_exponent_raises() raises:
    """A number with a glued second exponent (1e1e1) makes get_double raise."""
    ...
    try:
        ...
        raised = True
    assert_true(raised, "get_double on 1e1e1 must raise (trailing junk)")
...
def test_get_int_hex_prefix_raises() raises:
    ...
    var raised = False
    try:
        _ = root.find_field(String("n")).get_int()
    except:
        raised = True
...
def test_get_int_array_element_terminator() raises:
    ...
    var parser = Parser()
    var root = parser.iter(data)
    var arr = root.find_field(String("a")).get_array()
    assert_equal(arr.next_element().get_int(), Int64(7))


def test_get_double_negative_float() raises:
    """A negative float ending at `}` (-12.34) parses within 1e-9."""
    var data = _make_bytes(String('{"n":-12.34}'))
    var parser = Parser()
    var root = parser.iter(data)
    var got = root.find_field(String("n")).get_double()
    assert_true(
        (got - Float64(-12.34)) < Float64(1e-9)
        and (Float64(-12.34) - got) < Float64(1e-9),
        "get_double on -12.34 must be within 1e-9",
    )


def test_get_double_overflow_to_inf() raises:
    """A float beyond double range (1e999) ends cleanly and reads as inf."""
    var data = _make_bytes(String('{"n":1e999}'))
    var parser = Parser()
    var root = parser.iter(data)
    var got = root.find_field(String("n")).get_double()
    assert_true(isinf(got), "get_double on 1e999 must be inf")


def test_get_double_small_exponent() raises:
    ...
    assert_true(
        ...
        "get_double on 1.5e-3 must be within 1e-9 of 0.0015",
    )
...
def test_get_int_space_terminator_before_comma() raises:
    ...
    var parser = Parser()
    var root = parser.iter(data)
    assert_equal(root.find_field(String("n")).get_int(), Int64(42))


def main() raises:
    test_get_double_double_dot_raises()
    test_get_double_one_dot_two_dot_three_raises()
    test_get_double_double_exponent_raises()
    ...
    test_get_double_negative_float()
    test_get_double_overflow_to_inf()
    test_get_double_small_exponent()
