...
def _handle_escape(
    ...
    if i + 1 >= input_len:
        raise ParseError(code=ErrorCode.UNCLOSED_STRING.value, position=i)
...
def parse_string(
    ...
    while i < input_len:
        ...
        if first_ctrl < first_special:
            raise ParseError(code=ErrorCode.STRING_ERROR.value, position=i + first_ctrl)
...
def _parse_unicode_escape(
    ...
    if code_point >= 0xD800 and code_point <= 0xDBFF:
        ...
        if (
            ...
            raise ParseError(code=ErrorCode.STRING_ERROR.value, position=backslash_pos)
