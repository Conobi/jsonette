...
struct Value[o: Origin[mut=True]](Movable):
    ...
    def get_int(self) raises -> Int64:
        ...
        var input_len = self._reader[]._input_len
        var pos = Int(p.positions[self._si])
        var r = _parse_number(p.get_input_ptr() + pos, input_len - pos)
        ...
        if not _scalar_token_ok(
            p.get_input_ptr(), pos, r.bytes_consumed, input_len
        ):
    ...
    def get_bool(self) raises -> Bool:
        ...
        var input_len = self._reader[]._input_len
        var pos = Int(p.positions[self._si])
        var b = p.get_input_ptr()[pos]
        if b == _LOWER_T:
            _validate_true(p.get_input_ptr(), pos, input_len)
            if not _scalar_token_ok(p.get_input_ptr(), pos, 4, input_len):
                raise Error("get_bool: trailing characters after literal")
            return True
        if b == _LOWER_F:
            _validate_false(p.get_input_ptr(), pos, input_len)
            if not _scalar_token_ok(p.get_input_ptr(), pos, 5, input_len):
                raise Error("get_bool: trailing characters after literal")
            return False
    ...
    def is_null(self) raises -> Bool:
        ...
        var input_len = self._reader[]._input_len
        var pos = Int(p.positions[self._si])
        if p.get_input_ptr()[pos] != _LOWER_N:
            return False
        _validate_null(p.get_input_ptr(), pos, input_len)
        if not _scalar_token_ok(p.get_input_ptr(), pos, 4, input_len):
    ...
    def _first_byte(self) -> UInt8:
        ...
        ref p = self._reader[]._parser
        return p.get_input_ptr()[Int(p.positions[self._si])]
    ...
    def as_int(self) raises -> Optional[Int64]:
        ...
        var pos = Int(p.positions[self._si])
        var r = _parse_number(p.get_input_ptr() + pos, input_len - pos)
        if not _scalar_token_ok(p.get_input_ptr(), pos, r.bytes_consumed, input_len):
...
struct Field[o: Origin[mut=True]](Movable):
    ...
    def key(self) raises -> String:
        ...
        return _unescaped_key_into(
            p.get_input_ptr(), key_pos, self._reader[]._input_len, p._od_scratch
        )
...
struct Object[o: Origin[mut=True]](Movable):
    ...
    def _skip_value(self, value_si: Int) -> Int:
        ...
        ref p = self._reader[]._parser
        return _skip_value(
            p.get_input_ptr(), p.positions, len(p.positions), value_si
        )
    ...
    def at_end(self) -> Bool:
        ...
        if self._si >= n:
            return True
        var b = p.get_input_ptr()[Int(p.positions[self._si])]
        if b == _RBRACE:
            return True
...
struct Array[o: Origin[mut=True]](Movable):
    ...
    def at_end(self) -> Bool:
        ...
        if self._si >= n:
            return True
        return p.get_input_ptr()[Int(p.positions[self._si])] == _RBRACK
