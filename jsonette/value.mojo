...
struct Value[o: Origin[mut=True]](Copyable, Movable, Sized, Writable):
    ...
    def _str_byte(self, payload: UInt64, i: Int) -> UInt8:
        ...
        Raw spans read the parser's input buffer via get_input_ptr() (works for
        both copy and nocopy paths); buffer entries read the unescaped bytes in
        string_buf. The branch is perfectly predicted within any one string.
        """
        if is_raw_string(payload):
            return self._doc[]._parser.get_input_ptr()[raw_string_offset(payload) + i]
